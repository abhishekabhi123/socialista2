...
const Applications = () => {
  ...
  useEffect(() => {
    axios
      .get(`posts/reports/1`, { withCredentials: true })
      .then((res) => {
        setData(res.data);
    ...
    if (type == "post") {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .delete(`posts/${postId}/rejectReport?id=${item}&name=${email}`, {
  ...
  const actionColumn = [
    {
      ...
      renderCell: (params) => {
        ...
        return <img className="image" src={params.row.post} alt="no image" />;
      },
    },
    {
      field: "action",
